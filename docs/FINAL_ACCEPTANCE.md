...
|---|---|---|
| Debug and Release regression | The complete current suites pass 87/87 tests in Debug and Visual Studio Release. Ninja is not installed on this validation host, so no current Ninja result is claimed. | PASS (AUTOMATED MSVC); NINJA NOT RUN |
