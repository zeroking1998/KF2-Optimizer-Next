...
|---|---|---|
| Debug, Release and Ninja regression | The complete current suites contain 88 tests. Fresh focused and full regression results are recorded per pull request; Ninja availability is reported separately when applicable. | PASS (AUTOMATED MSVC); NINJA ENVIRONMENT-DEPENDENT |
...
| Protected gameplay telemetry and adaptive corpse load control | Official KFEditor compiles the pinned package with 0 errors/0 warnings; telemetry stays read-only. A normal app-started session prepares the hash-bound corpse provider automatically. Adaptive corpse capacity, cleanup, distance physics and visible-ragdoll sleep remain pressure-controlled. Separately, a bounded controller fixes eligible living-Zed/corpse meshes and living animation inputs to their lowest safe engine-native levels regardless of Adaptive state; sleeping corpses use KF2's native final-pose flags. Direct living bone arrays, attacks, hits, physics, collision and culling remain unchanged. Source-contract tests require exact fixed-minimum readback and verify that Adaptive-off does not restore these world-lifetime values. The new policy still needs a fresh isolated gameplay run. | PASS CONTRACT; PENDING COMPILE/LIVE ISOLATION |
...
| DXGI/CPU/GPU/overlay contracts | Current Release tests cover native DXGI presents, freshness, total-process CPU, busiest-thread occupancy, effective logical-core use, active threads, dominant-thread share, physical/logical affinity capacity, physical PnP GPU grouping, Afterburner-compatible NVIDIA NVAPI utilization, local NVML and adapter-wide PDH fallbacks, and overlay soak. Affinity is observed but never changed. | PASS (AUTOMATED) |
...
| Offline FleX laboratory | 37 direct PE forwarders plus fifteen local ABI relays, including exact set/wait fence, solver bounds, parameter-call and six particle/phase/velocity bulk-transfer observations, fail-closed identity gate and transactional restore pass automatically. User-disabled FleX remains untouched. User-enabled FleX requests the fixed minimum of one solver substep independently of Adaptive and becomes `APPLIED` only after current-generation shared-memory requested/forwarded readback, recorded as `FLEX_MINIMUM_APPLIED`. The original runtime and configuration remain transactionally protected and restored. | PASS (AUTOMATED); PENDING FIXED-MINIMUM LIVE RUN |
...
| No Windows system sounds | Sound controls and callbacks are removed; the Release PE is tested to contain no `PlaySoundW`, `MessageBeep`, Windows sound aliases or `winmm.dll` import. The UI uses a normal 16 ms Windows animation timer. | PASS (AUTOMATED) |
