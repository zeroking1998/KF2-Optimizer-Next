...
| Protected gameplay telemetry and adaptive corpse load control | Official KFEditor compiles the pinned package with 0 errors/0 warnings; telemetry stays read-only. A normal app-started General Adaptive session now prepares the hash-bound corpse provider automatically even when the legacy optional lab switch is off; safe/read-only starts retain fail-closed behavior. When available, the independently reported protected corpse capability keeps the selected 4-2000 count as the stable ceiling and uses official confirmed-corpse cleanup at most once per 450 ms only while a fresh current sample and sustained target-relative pressure are present. Confirmed pressure lowers a separate runtime threshold stepwise; current recovery stops removal and restores capacity gradually toward the selected ceiling. Distant corpse physics is now distance-first: old slow offscreen/occluded bodies beyond 3,500 units may sleep once per second without FPS pressure, while visible living/corpse density or confirmed frame pressure moves entry to 2,500/1,800 units and 500/250 ms. Identity tracking remains capped at 32 and proximity wake remains 1,500 units. Reversible corpse LOD, bounded visible-ragdoll sleep and cleanup remain frame-pressure-only. Native living-enemy frustum/occlusion culling is retained with no manual hiding. The source-contract test proves that the Zed-Time guard precedes wake and sleep, proximity wake releases identity only after the native wake call, and the distance stage precedes the frame-only action gate. The prior protected Biotics Lab combat run observed live living/corpse telemetry and runtime-budget receipts, but the automatic-start change and new distance/density policy still need a fresh isolated gameplay run. | PASS COMPILE/CONTRACT; PENDING NEW POLICY LIVE ISOLATION |
...
| Fresh GUI start/exit | The final portable package starts visibly, enforces single instance and exits cleanly; damaged-package and loose-EXE starts enter fail-closed Safe Mode | PASS (AUTOMATED/VISUAL) |
