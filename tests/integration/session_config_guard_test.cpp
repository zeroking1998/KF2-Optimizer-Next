...
int main() {
    ...
    CHECK(snapshot.has_value()); CHECK(snapshot.value().file_count == 2);
    ...
    CHECK(restored.has_value() && restored.value() == 2);
    ...
    CHECK(resumed.value()->file_count == 3);
    ...
    CHECK(recovered.has_value() && recovered.value() == 3);
    CHECK(read(config / L"KFEngine.ini") == "engine-original");
