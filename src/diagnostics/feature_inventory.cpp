...
constexpr AreaContract area06{
    ...
    "Global rendering controls are reversible; the offline probe observes actual living-enemy LOD, render recency, material slots and mesh attachments. A separate bounded session controller fixes eligible living Zeds and corpses to each mesh's final available MinLodModel without hiding them, while the authenticated graphics actuator retains the verified global skeletal-mesh LOD path.",
...
constexpr std::array area06_items{
    ItemSpec{FeatureStatus::partial, "skeletal-mesh LOD", "The authenticated live actuator may temporarily raise the global bias under CPU pressure. Separately, the fixed-minimum session controller applies each eligible living-Zed and corpse mesh's final available MinLodModel regardless of Adaptive mode, FPS, distance, density or pressure, with exact readback and ownership-aware tracking."},
...
constexpr AreaContract area07{
    ...
    "The offline probe observes animation LOD, required bones, native skip/interpolation, offscreen tick and kinematic-distance state. A separate fixed-minimum session controller applies AnimationLODDistanceFactor 0.55 and AnimationLODFrameRate 6 to eligible living monsters; KF2 alone owns its const transient skip/cache/interpolation flags.",
...
constexpr std::array area07_items{
    ItemSpec{FeatureStatus::partial, "animation update rate by distance", "Eligible living Zeds use the fixed lowest safe documented inputs AnimationLODDistanceFactor 0.55 and AnimationLODFrameRate 6 throughout the protected session. The target does not depend on Adaptive mode or observed pressure, and unchanged values produce no redundant write or receipt."},
    ItemSpec{FeatureStatus::partial, "animation LOD and bone LOD", "PredictedLODLevel and RequiredBones totals are observed. The fixed controller changes only render MinLodModel and KF2's documented animation-LOD inputs; direct RequiredBones mutation remains excluded."},
    ...
    ItemSpec{FeatureStatus::partial, "visibility- and occlusion-based evaluation", "Visibility remains telemetry only for this feature. Fixed-minimum render and animation LOD is applied with bounded scans; enemies are never hidden and native culling remains untouched."},
...
constexpr AreaContract area09{
    ...
    "Adaptive profiles for cosmetic budgets, distant-corpse physics with proximity wake, bounded one-at-a-time cleanup and pressure-triggered visible-ragdoll sleep; a separate fixed-minimum session controller owns corpse/living visual LOD and safe sleeping-corpse skeleton reduction. Sensitive global collision switches remain protected from Adaptive writes.", "Automatic protected launch transaction, preview, session snapshot and backup restore.",
...
constexpr std::array area09_items{
    ...
    ItemSpec{FeatureStatus::partial, "maximum active ragdolls", "Official telemetry reports total and visible awake-ragdoll pressure. No fixed corpse-count cap is imposed: instead, every eligible awake corpse is processed each 250 ms, settled bodies sleep after 0.75 seconds and full rigid-body simulation is hard-limited to 2 seconds. Distance, LOD and pressure stages remain additional reductions."},
    ItemSpec{FeatureStatus::partial, "sleep and rest state", "The protected corpse capability gives every confirmed monster corpse a short full-physics reaction window, sleeps settled awake bodies after 0.75 seconds and hard-limits any remaining awake rigid-body simulation to 2 seconds. Every eligible corpse is processed on each 250 ms control pass without a corpse-count or batch limit. This baseline is independent of FPS, quality, distance, density, enemy pressure and the selected corpse maximum. The distance layer additionally sleeps eligible bodies beyond 1,200 units, or 1,000/850 units under pressure, while proximity wake uses 800 units. Broader manual mutation remains unavailable."},
    ...
    ItemSpec{FeatureStatus::partial, "update rate and distance thresholds", "Actual corpse LOD and official MaxCorpseOffscreenTime/Distance are reported. A pressure-independent baseline first sleeps settled awake corpses after 0.75 seconds and force-sleeps remaining awake corpse rigid bodies after 2 seconds. The distance actuator additionally sleeps old slow offscreen/occluded bodies beyond 1,200 units at baseline; visible enemy/corpse density or confirmed frame pressure may move entry to 1,000/850 units while continuously rising enemy pressure shortens action intervals to a 50 ms floor. A tracked distance-slept body wakes inside 800 units. Separately, a fixed session controller applies final available corpse/living MinLodModel, living AnimationLODDistanceFactor 0.55 and AnimationLODFrameRate 6 independently of Adaptive mode and pressure. Sleeping corpses use native final-pose flags; KF2 remains authoritative for living transient bone-skip, interpolation and cache outputs."},
    ...
    ItemSpec{FeatureStatus::present, "Adaptive values with complete restore", "Adaptive uses verified cosmetic budgets, protected staged cleanup and pressure-triggered corpse sleep. Fixed-minimum Zed/corpse LOD and safe sleeping-corpse skeleton reduction run separately for the world lifetime; sensitive shipped controls remain blocked."},
};
...
constexpr std::array area10_items{
    ...
    ItemSpec{FeatureStatus::partial, "translucency, overdraw, lighting, shadows, and render targets", "Fresh visible-particle occupancy and active decal saturation feed a conservative overdraw signal. Verified standalone pressure uses the complete reversible live group. A verified joined/listen-server session can use only the isolated process-local GPU, VRAM, RAM, overdraw and effects groups with exact readback; CPU/mixed and every gameplay-actor group remain blocked. Internal render-target allocation is not exposed."},
