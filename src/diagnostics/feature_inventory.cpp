...
constexpr AreaContract area09{
    ...
    "Corpse controls are reversible; official telemetry reports pool capacity, awake/sleep/final-pose, visibility, LOD, injury, age and runtime offscreen/lifetime rules. When the protected capability is available, distance sleeps eligible confirmed monster corpses permanently, with pressure-scaled batches; LOD, cleanup and visible-ragdoll stages follow only if needed.",
...
constexpr std::array area09_items{
    ...
    ItemSpec{FeatureStatus::partial, "maximum active ragdolls", "Official telemetry reports total and visible awake-ragdoll pressure. The protected corpse capability may sleep one eligible body per baseline pass or pressure-scaled batches of at least two/four, then reduces distant-corpse mesh LOD or sleeps an old visible body if needed; no fixed global active-ragdoll cap is imposed."},
    ...
    ItemSpec{FeatureStatus::partial, "update rate and distance thresholds", "Actual corpse LOD and official MaxCorpseOffscreenTime/Distance are reported. The distance-first actuator enters at 1,500 units at baseline; visible-scene density or confirmed frame pressure may move entry to 1,100/750 units and permit batches of at least two/four per pass. Reversible MinLodModel uses a stable 1,000/900-unit apply/restore band with actor-correlated restore receipts; external overrides are released and excluded from reacquisition."},
