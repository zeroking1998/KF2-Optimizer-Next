...
constexpr std::array area05_items{
    ...
    ItemSpec{FeatureStatus::present, "ragdolls and detached limbs", "Official read-only snapshots expose awake/sleeping rigid bodies, bNoSkeletonUpdate final pose, bHasBrokenConstraints corpses, exact HitZones.bPlayedInjury detached-limb totals and ragdoll-insomnia warning level. When its protected capability is available, distance and scene or target-relative frame pressure permanently sleep eligible confirmed monster corpses."},
...
constexpr AreaContract area09{
    ...
    "Corpse controls are reversible; official telemetry reports pool capacity, awake/sleep/final-pose, visibility, LOD, injury, age and runtime offscreen/lifetime rules. When the protected capability is available, one linear distance pass may permanently sleep every eligible confirmed monster corpse in the user-sized pool; LOD, cleanup and visible-ragdoll stages follow only if needed.",
    ...
    "Adaptive profiles for cosmetic budgets, permanent distance-based corpse sleep, reversible distant-corpse LOD, bounded one-at-a-time cleanup and pressure-triggered visible-ragdoll sleep; sensitive global collision switches remain protected from Adaptive writes.", "Automatic protected launch transaction, preview, session snapshot and backup restore.",
...
constexpr std::array area09_items{
    ...
    ItemSpec{FeatureStatus::partial, "maximum active ragdolls", "Official telemetry reports total and visible awake-ragdoll pressure. The protected corpse capability may sleep every currently eligible distant body in one linear pool pass, then reduces distant-corpse mesh LOD or sleeps an old visible body if needed; no second fixed Distance-Sleep actor cap is imposed."},
    ItemSpec{FeatureStatus::partial, "sleep and rest state", "The protected corpse capability uses distance, native render recency, age, motion, corpse density and independently calculated visible-enemy pressure, then uses the official PutRigidBodyToSleep path for eligible confirmed dead monster corpses. The FPS-independent baseline enters at 1,200 units; corpse, dynamic enemy or confirmed frame pressure moves entry to 800/450 units. Distance Sleep is permanent for that corpse and Near Wake is removed. Broader manual mutation remains unavailable."},
    ItemSpec{FeatureStatus::partial, "transition to a final pose", "After the official sleep transition, bNoSkeletonUpdate and bSkipAllUpdateWhenPhysicsAsleep mirror KF2's own Dying.OnSleepRBPhysics optimization for the remaining corpse lifetime."},
    ItemSpec{FeatureStatus::partial, "update rate and distance thresholds", "Actual corpse LOD and official MaxCorpseOffscreenTime/Distance are reported. The distance-first actuator enters at 1,200 units at baseline; corpse density, dynamic enemy pressure or confirmed frame pressure may move entry to 800/450 units. Enemy pressure is a monotonic 0-100 score from visible enemies, total living enemies and active attacks; levels begin at 30/65 and the exact score continuously moves Distance Sleep toward 450 units. The selected maximum bounds the pool, and every currently eligible body may sleep in one linear pass. Reversible MinLodModel uses a stable 1,000/900-unit apply/restore band with actor-correlated restore receipts; external overrides are released and excluded from reacquisition."},
