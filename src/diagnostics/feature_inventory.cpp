...
constexpr std::array area09_items{
    ...
    ItemSpec{FeatureStatus::partial, "sleep and rest state", "The protected corpse capability uses distance, native render recency, age, motion and visible living/corpse density as primary inputs, then uses the official PutRigidBodyToSleep path for one eligible confirmed dead monster corpse. A conservative offscreen-only 2,800-unit baseline works independently of FPS; density or confirmed frame pressure moves entry to 2,300/1,900 units, while proximity wake uses 750 units. Broader manual mutation remains unavailable."},
    ...
    ItemSpec{FeatureStatus::partial, "update rate and distance thresholds", "Actual corpse LOD and official MaxCorpseOffscreenTime/Distance are reported. The distance-first actuator sleeps only old slow offscreen/occluded bodies beyond 2,800 units at baseline; visible-scene density or confirmed frame pressure may move entry to 2,300/1,900 units and the interval from 1,000 to 500/250 ms. A tracked body wakes inside 750 units. Reversible MinLodModel uses a stable 1,000/900-unit apply/restore band and native culling is never overridden."},
