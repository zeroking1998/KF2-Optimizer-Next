...
constexpr std::array area09_items{
    ...
    ItemSpec{FeatureStatus::partial, "sleep and rest state", "The protected corpse capability uses distance, native render recency, age, motion and visible living/corpse density as primary inputs, then uses the official PutRigidBodyToSleep path for eligible confirmed dead monster corpses. The FPS-independent baseline enters at 1,500 units; density or confirmed frame pressure moves entry to 1,100/750 units. Distance Sleep is permanent for that corpse and Near Wake is removed. Broader manual mutation remains unavailable."},
    ...
    ItemSpec{FeatureStatus::partial, "update rate and distance thresholds", "Actual corpse LOD and official MaxCorpseOffscreenTime/Distance are reported. The distance-first actuator enters at 1,500 units at baseline; visible-scene density or confirmed frame pressure may move entry to 1,100/750 units. The selected maximum bounds the pool, and every currently eligible body may sleep in one linear pass. Reversible MinLodModel uses a stable 1,000/900-unit apply/restore band with actor-correlated restore receipts; external overrides are released and excluded from reacquisition."},
