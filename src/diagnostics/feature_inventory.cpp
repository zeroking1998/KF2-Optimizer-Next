...
constexpr std::array area06_items{
    ItemSpec{FeatureStatus::partial, "skeletal-mesh LOD", "The authenticated live actuator may temporarily raise the global bias under CPU pressure. Separately, fresh visible-enemy pressure scales continuously from five to eighty weighted Zeds; recently rendered enemies inside 1,200/600 units add one/two extra pressure points. It may raise living-monster MinLodModel from 600 toward 300 units, consuming every real mesh LOD available while preserving exact readback, near restore and full session restore; corpse MinLodModel remains independently reversible."},
...
constexpr std::array area07_items{
    ItemSpec{FeatureStatus::partial, "animation update rate by distance", "Fresh visible-enemy pressure scales continuously across the 5-80 visible-Zed range. It raises AnimationLODDistanceFactor from 0.15 through 0.55, AnimationLODFrameRate from 2 through 6 and moves the activation distance from 600 toward 300 units; sufficiently distant actors gain one final rate step. Exact readback, ownership-aware near restore and session restore are required."},
...
constexpr std::array area09_items{
    ...
    ItemSpec{FeatureStatus::partial, "sleep and rest state", "The protected corpse capability uses distance, native render recency, age, motion and visible living/corpse density as primary inputs, then uses the official PutRigidBodyToSleep path for one eligible confirmed dead monster corpse. An offscreen-only 1,200-unit baseline works independently of FPS; enemy, density or confirmed frame pressure moves entry to 1,000/850 units, while proximity wake uses 800 units. Broader manual mutation remains unavailable."},
