...
constexpr std::array area06_items{
    ItemSpec{FeatureStatus::partial, "skeletal-mesh LOD", "The authenticated live actuator may temporarily raise the global bias under CPU pressure. Separately, one visible living Zed starts a small visual-only baseline and fresh visible-enemy pressure scales continuously to eighty weighted Zeds; recently rendered enemies inside 1,200/600 units add one/two extra pressure points. It may raise living-monster MinLodModel from about 585 toward 300 units, consuming every real mesh LOD available while preserving exact readback, near restore and full session restore; corpse MinLodModel remains independently reversible."},
...
constexpr std::array area07_items{
    ItemSpec{FeatureStatus::partial, "animation update rate by distance", "One visible living Zed starts a small visual-only baseline and fresh enemy pressure scales continuously through eighty weighted Zeds. It raises AnimationLODDistanceFactor from about 0.17 through 0.55, AnimationLODFrameRate from 2 through 6 and moves the activation distance from about 585 toward 300 units; sufficiently distant actors gain one final rate step. Exact readback, ownership-aware near restore and session restore are required."},
